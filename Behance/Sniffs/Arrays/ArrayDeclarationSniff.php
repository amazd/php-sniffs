    ...
    if ( $tokens[$arrayStart]['line'] === $tokens[$arrayEnd]['line'] ) {
      ...
      while ( ( $nextArrow = $phpcsFile->findNext( T_DOUBLE_ARROW, ( $nextArrow + 1 ), $arrayEnd ) ) !== false ) {
        if ( $tokens[$nextArrow - 1]['code'] !== T_WHITESPACE ) {
          ...
          $data       = [$content];
          $should_fix = $phpcsFile->addFixableError( $error, $nextArrow, 'NoSpaceBeforeDoubleArrow', $data );
          if ( $should_fix ) {
            $phpcsFile->fixer->replaceToken( $nextArrow, ' ' . $tokens[$nextArrow]['content'] );
          }
        ...
        if ( $tokens[($nextArrow + 1)]['code'] !== T_WHITESPACE ) {
          ...
          $data       = [$content];
          $should_fix = $phpcsFile->addFixableError( $error, $nextArrow, 'NoSpaceAfterDoubleArrow', $data );
          if ( $should_fix ) {
            $phpcsFile->fixer->replaceToken( $nextArrow, $tokens[$nextArrow]['content'] . ' ' );
          }
        } // if nextArrow + 1 !== T_WHITESPACE
      ...
      if ( $valueCount > 0 ) {
        ...
        foreach ( $commas as $comma ) {
          if ( $tokens[$comma + 1]['code'] !== T_WHITESPACE ) {
            ...
            if ( $should_fix ) {
              $phpcsFile->fixer->replaceToken( $comma, $tokens[$comma]['content'] . ' ' );
            }
